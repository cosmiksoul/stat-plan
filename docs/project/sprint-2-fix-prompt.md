...
- `docs/project/test-cases-sprint-2.md` — секция Bugs found (BUG-1, BUG-2, BUG-3)
- `docs/project/code-review-sprint-2.md` — Concern #1 (Decision Log)
...
**Откуда:** `docs/project/code-review-sprint-2.md` Concern #1 + Decision Log решение «Sprint 2 fix» (пересмотрено после QA с пользователем).
...
В `vite.config.js` появилось поле `server.watch.ignored: ['**/docs/**', '**/mockups/**', '**/tests/**']` — это **правка Cowork'а во время QA-фазы**. Причина: пользователь редактировал `docs/project/test-cases-sprint-2.md` параллельно с тестированием, и Vite триггерил full reload приложения, сбрасывая state. Эта правка решает проблему.
...
После завершения дополни **существующий** `docs/project/sprint-report-2.md` секцией «## Bug Fixes (FIX phase)» в конце (или создай отдельный `docs/project/sprint-2-fix-report.md` — на твоё усмотрение, см. Dev-Cycle.md). В отчёте:
