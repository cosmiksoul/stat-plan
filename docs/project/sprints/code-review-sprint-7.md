...
✅ **Cowork-зона не тронута** (exception: `docs/project/sprints/sprint-report-7.md` — Code-exception per CLAUDE.md P-1).
...
> Sprint 7 готов к QA. **Тест-кейсы + 6 готовых тестовых `.ipynb` файлов** в `docs/project/sprints/test-cases-sprint-7.md` — drag-drop их на /step4 для разных сценариев (full happy path, KILL scenario, SRM fail, backward-compat без tag, partial fields, broken JSON). Особое внимание:
...
- `docs/project/sprints/sprint-report-7.md` — отчёт Code (детально про D1-D7 decisions, time tracking)
- `docs/project/sprints/sprint-7-prompt.md` — prompt (S1-S13 spec)
- `docs/context/decisions-log.md` — ADR-015 (ipynb export format)
- `docs/project/sprints/test-cases-sprint-7.md` — runnable smoke (создаётся)
