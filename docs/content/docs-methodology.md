...
**Скоринг** проверяет соответствие выбранного метода типу метрики (группа «Методологическая консистентность» в оценке тест-плана даёт 30 баллов). Если выбрал `t_test` для proportion — увидишь предупреждение и часть очков отнимется.
